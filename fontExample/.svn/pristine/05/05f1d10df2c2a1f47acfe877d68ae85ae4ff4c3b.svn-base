...
$(document).ready(function() {
	...
	$("<div>").text("구매내역").attr("id","e").appendTo($body);
	...
	function printList(page) {
		...
		$.each(page.summarys, function(idx, sm) {
			...
			var $tradeDate = $("<p>").text("거래날짜 : ").appendTo($buyDate);
			// 거래날짜
			...
			// 체크박스
			$("<input>").attr("type","checkbox").attr("class","sno").val(sm.no).appendTo($buyOrderCheckbox);
			...
			// 판매자
			$("<span>").text("판매자 : " + sm.sellerUsername).appendTo($orderProductInfo).css("cursor","pointer").on("click",function() {
			...
			$("<button>").attr("id","reviewBtn").text("리뷰 쓰기").attr("class","btn btn-default")
				.on("click",function() {
					location.href="/fontExample/sellReviewReport?username="+sm.sellerUsername+"&productName="+sm.name;
				}).appendTo($orderBtn);
			$("<button>").attr("id","reportBtn").text("신고하기").attr("class","btn btn-default")
				.on("click",function() {
					location.href="/fontExample/declaration?no="+sm.no;
				}).appendTo($orderBtn);
		});
	...
	function printPage(page) {
		...
		if (blockNo > 1) {
			...
			$("<a>").attr("href",
					"/fontExample/buy_history?pageno=" + (startPage - 1)).text(
					"앞으로").appendTo($li);
		}
		for (var i = startPage; i <= endPage; i++) {
			if (i == page.pageno) {
				var $li = $("<li>").attr("class", "active").appendTo($ul);
				$("<a>").attr("href", "/fontExample/buy_history?pageno=" + i)
						.text(i).appendTo($li);
			} else {
		...
		if (endPage < cntOfPage) {
			...
			$("<a>").attr("href",
					"/fontExample/buy_history?pageno=" + (endPage + 1)).text(
					"다음으로").appendTo($li);
		}
	}
	...
	$(function() {
		$.ajax({
			url : "/fontExample/api/buy_historys",
			method : "get",
