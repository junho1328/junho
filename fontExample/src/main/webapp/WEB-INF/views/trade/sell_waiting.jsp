...
$(document).ready(function() {
	...
	$("<div>").text("판매대기상품").attr("id","e").appendTo($body);
	...
	function printList(page) {
		...
		$.each(page.purchases, function(idx, sm) {
			...
			// 판매자
			$("<span>").text("구매자 : " + sm.username).appendTo($orderProductInfo).css("cursor","pointer").on("click",function() {
			...
			// 거래상태
			if(sm.state==1) {
				$("<p>").text("구매 예약").appendTo($orderStatusMessage);
			} else {
				$("<p>").text("판매확인 완료").appendTo($orderStatusMessage);				
			}
			...
			$("<button>").attr("id","reviewBtn").val(sm.state).text("판매 완료").attr("class","btn btn-warning")
				.on("click",function() {
					if($(this).val()==1)
						purchaseFinal()
					else 
						alert("이미 판매완료한 상품입니다.");
				}).appendTo($orderBtn);
			$("<button>").attr("id","reportBtn").val(sm.state).text("예약 취소").attr("class","btn btn-default")
				.on("click",function() {
					//cancelPurchase()
				}).appendTo($orderBtn);
	...
	function purchaseFinal(){
			...
			$.ajax({
			url : "/fontExample/api/purchase/sell/ok",
			method : "post",
			data : param,
			success : function(result){
			alert("판매확정 완료")
				location.href = "http://localhost:8081/fontExample/sell_waiting"
			}
		})
	}
	function printPage(page) {
		...
		if (blockNo > 1) {
			...
			$("<a>").attr("href",
					"/fontExample/sell_waiting?pageno=" + (startPage - 1)).text(
					"앞으로").appendTo($li);
		}
		for (var i = startPage; i <= endPage; i++) {
			if (i == page.pageno) {
				var $li = $("<li>").attr("class", "active").appendTo($ul);
				$("<a>").attr("href", "/fontExample/sell_waiting?pageno=" + i)
						.text(i).appendTo($li);
			} else {
		...
		if (endPage < cntOfPage) {
			...
			$("<a>").attr("href",
					"/fontExample/sell_waiting?pageno=" + (endPage + 1)).text(
					"다음으로").appendTo($li);
		}
	}
	...
	$(function() {
		$.ajax({
			url : "/fontExample/api/purchase/waiting/seller",
			method : "get",
